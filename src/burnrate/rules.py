...
def burn_rate_rules(slo: SLO, windows: Optional[List[dict]] = None) -> dict:
    ...
    for tier, ws in by_tier.items():
        ...
        for w in ws:
            ...
            # plain replace, not str.format — the query is full of PromQL `{...}`
            lq = slo.error_query.replace("{window}", duration_str(w["long_h"]))
            sq = slo.error_query.replace("{window}", duration_str(w["short_h"]))
